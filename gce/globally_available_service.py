...
def gen_target_pool(name, health_checks):
    return {
        'name': name + '-target-pool',
...
def gen_global_forwarding_rule(name, target_proxy, port, ip_address=None):
    return {
        ...
        'type': 'compute.v1.globalFowardingRule',
        'properties': {
...
def gen_backend_service(name,
                        ...
                        https=False):
    backends = [
        dict(group=SELF_LINK.format(igm['name']), **backend_template)
        for igm in igms
    ]
    return {
...
def gen_instance_group_manager(name,
                               ...
                               target_pool=None):
    return {
        'name': name + '-igm-' + instance_template['properties']['zone'],
...
def GenerateConfig(context):
    ...
    # Only need a target pool if the user wants a health check
    health_check = None
    target_pool = None
    if health_check_template:
        health_check = gen_health_check(
            name, health_check_template, port, ssl=cert)
        target_pool = gen_target_pool(name, [health_check])
    ...
    for zone in context.properties['zones']:
        igm = gen_instance_group_manager(
            name, zone, instance_template, target_pool=target_pool)
        igms.append(igm)
        resources.append(igm)

    # Optionally autoscale those instances
